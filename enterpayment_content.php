	...
	<table><tr><td>
		...
		if($_SESSION['USERNAME'] == "steppsr" or $_SESSION['USERNAME'] == "westcrabtree" or $_SESSION['USERNAME'] == "russdog" or $_SESSION['USERNAME'] == "gofasta")
		{
			...
			{	// if submitted then we should update the tables
				...
				$sql = "SELECT balance,memberspaid FROM members WHERE id='" . $_POST['member'] . "' LIMIT 1;--";
				...
				{	$membersbalance = $row['balance'] + $_POST['amount'];
					$memberspaid = $row['memberspaid'] + $_POST['amount'];
					$membersid = $_POST['member'];
					
					$sql = "UPDATE members SET memberspaid='" . $memberspaid . "', balance='" . $membersbalance . "' WHERE id='" . $membersid . "';--";
					mysql_query($sql);

					$sql = "INSERT INTO ledger (amount,member_id,dt,type) VALUES ('" . $_POST['amount'] . "', '" . $membersid . "', NOW(), '2');--";
					mysql_query($sql);
			...
			else	// if not submitted, then user just came to the page and we should display the entry form
			{
				...
				1. The member's balance is increased by the amount entered.<br>
				2. The member's paid amount is increased by the amount entered.<br>
				3. An entry is created in the ledger for the member with the amount, current date & time, and the type recorded. (The type is a payment)<br>
				<br>
